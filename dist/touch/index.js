...
export function initInputTouch(element, state, options = {}) {
    ...
    const touchIdentifierMap = new Map(); // <touch.identifier, our 0-9 index>
    const touchLastMoveTime = new Map(); // <touchIndex, timestamp>
    const touchStopCheckRequestIds = new Map(); // <touchIndex, animationFrameId>
    let touchX = 0, touchY = 0; // State variables for coordinates
    const updateCoords = (t) => {
    ...
    const handleTouchStart = (e) => {
        ...
        for (let i = 0, len = touches.length; i < len; i++) {
            ...
            if (!touch)
                continue;
            if (availableTouchIndices.length === 0)
                continue; // Max touches reached
            const touchIndex = availableTouchIndices.pop();
            touchIdentifierMap.set(touch.identifier, touchIndex);
            const keyId = (KeyTouch0 + touchIndex);
            updateCoords(touch);
    ...
    const handleTouchEndOrCancel = (e) => {
        ...
        for (let i = 0, len = touches.length; i < len; i++) {
            ...
            updateCoords(touch);
            const keyId = (KeyTouch0 + touchIndex);
            const moveKeyId = (KeyTouchMove0 + touchIndex);
    ...
    const checkTouchMoveStop = (touchIndex) => {
    ...
    const handleBlur = () => {
        ...
        for (const id of touchStopCheckRequestIds.values()) {
        ...
        for (let i = 9; i >= 0; i--)
            availableTouchIndices.push(i);
    };
    const opts = { passive: false };
    element.addEventListener('touchstart', handleTouchStart, opts);
    element.addEventListener('touchend', handleTouchEndOrCancel, opts);
    element.addEventListener('touchcancel', handleTouchEndOrCancel, opts);
    ...
    const destroy = () => {
        ...
        element.removeEventListener('touchend', handleTouchEndOrCancel, false);
        element.removeEventListener('touchcancel', handleTouchEndOrCancel, false);
...
export function isTouchKey(keyId) {
