 ...
 * @internal
 */
function updateBipolarSensorState(state, value, posKeyId, negKeyId, maxValue, deadzone) {
    if (value === null)
        value = 0; // Treat null as 0 to release keys if sensor stops reporting
    // --- Positive side ---
 ...
 * @internal
 */
function updateUnipolarSensorState(state, keyId, value, maxValue = 1) {
 ...
 * @internal
 */
function updateNormalizedSensorState(state, keyId, value, minValue, maxValue) {
 ...
 */
export function initInputSensor(state, options = {}) {
    const { sensorDeadzone = 0.1 } = options;
    const destroyers = [];
    const sensorOptions = { frequency: 60 };
    ...
    if (typeof LinearAccelerationSensor === 'function') {
        try {
            ...
            console.warn("Could not initialize LinearAccelerationSensor.", e);
        }
    }
    ...
    if (typeof Accelerometer === 'function') {
        try {
            ...
            console.warn("Could not initialize Accelerometer.", e);
        }
    }
    ...
    if (typeof Gyroscope === 'function') {
        try {
            ...
            console.warn("Could not initialize Gyroscope.", e);
        }
    }
    // --- Orientation ---
    if (typeof AbsoluteOrientationSensor === 'function') {
        try {
            ...
            const onReading = () => {
                if (!orient.quaternion || orient.quaternion.length < 4)
                    return;
            ...
            console.warn("Could not initialize AbsoluteOrientationSensor.", e);
        }
    }
    return { destroy: combineDestroyers(...destroyers) };
}
/** Detects if a key ID belongs to a sensor key. */
export function isSensorKey(keyId) {
